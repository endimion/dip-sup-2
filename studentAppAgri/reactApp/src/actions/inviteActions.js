...
export function sendValidation(_code,invHash){
  return function(dispatch){
    ...
    // axios.post("/agrBack/supplement/rest/invite/"+invHash+"/authorize",
    //                 { validationCode:_code,
    //                   inviteHash: invHash
    //           })
              retryAxiosNtimesPost(4,0,"/agrBack/supplement/rest/invite/"+invHash+"/authorize", { validationCode:_code,inviteHash: invHash})
              .then(res =>{
...
export function  getInvAndGenValCode(inviteId) {
  return  function(dispatch){
      ...
      retryAxiosNtimes(4,0,"/agrBack/supplement/rest/invite/"+inviteId)
      // axios.get("/agrBack/supplement/rest/invite/"+inviteId)
         .then(response =>{
           ...
           return promise;
            ...
            if(recipient!== ""){
                ...
                retryAxiosNtimes(4,0,"/agrBack/supplement/rest/view/"+id)
                // axios.get("/agrBack/supplement/rest/view/"+id)
                .then( resp =>{
                  dispatch({type: "GET_INV_SUP_FULLFILED", payload:JSON.parse(resp.data)});
                });
            }else{
                dispatch({type: "SEND_VAL_CODE"});
                retryAxiosNtimesPost(4,0,"/agrBack/supplement/rest/invite/"+inviteId+"/sendMail")
                // axios.post("/agrBack/supplement/rest/invite/"+inviteId+"/sendMail")
                .then(resp =>{
...
export function  shareByMail(_supId,_email) {
  return  function(dispatch){
      ...
      retryAxiosNtimesPost(4,0,"/agrBack/supplement/rest/inviteByMail",data)
...
export function  shareByQR(_supId,_email) {
  ...
  dispatch({type: "SHARE_SUP_QR"});
      retryAxiosNtimesPost(4,0,"/agrBack/supplement/rest/inviteByQR",{"supId":_supId, "email":_email})
